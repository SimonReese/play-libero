...
def load_environment(task: Task, BDDL_FILE = "") -> ControlEnv:
    # Load bddl file
    if BDDL_FILE == "":
...
def main():
    ...
    task_env = load_environment(None, CUSTOM_TASK)
    # Reset env
    task_env.seed(0)
    ...
    if VIDEO: save_video(frame_buffer, filename=f"{task_env.problem_name}.mp4")
