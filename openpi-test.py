...
HEADLESS = False
VIDEO = True
VIDEO_PATH = "./videos/openpi/cameras/"
MAX_STEPS = 600
...
def load_task(benchmark_suite: str, task_id: int) -> Tuple[Benchmark, Task]:
    ''' Load a spedfic task from a specific benchmark of LIBERO
        ...
        task_id: int
            The id of the task toFalse load
...
def main():
    # Load task
    bench, task = load_task(BENCHMARK_SUITE, 0)
    ...
    for step in range(MAX_STEPS):
        ...
        # Prepare images
        front_img = numpy.ascontiguousarray(obs["sideview_image"][::-1, ::-1])
